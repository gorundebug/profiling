...
Последующее изменение HTTP callback и проверка общих примитивов Boost описаны в [отдельном отчёте](runtime-hot-path-followup.md). Он уточняет приведённый ниже первоначальный итог: переход callback на общий объект позднее разрешён пользователем и реализован; mutex SingleUseEvent удалён, кандидат RotatingMap отклонён после stress-test.
